        ...
        <div class="breadcrumb-line breadcrumb-line-light header-elements-md-inline">
            <div class="d-flex">
                <div class="breadcrumb">
                    ...
                    <a href="{{ route('billPurchase.index') }}" class="breadcrumb-item"> فواتير المشتريات</a>
                    <span class="breadcrumb-item active">تعديل الفاتورة</span>
                    <span class="breadcrumb-item active">{{ $billPurchase->code }}</span>
                </div>
...
@section('content')
    <div id="tabstrip">
        ...
        <div>
            <div class="card">
                <div class="card-body">
                    <form action="{{ route('billPurchase.update' , $billPurchase->id) }}" method="post">
                        @csrf
                        @method('PUT')
                        <div class="row">
                            <div class="col-lg-3">
                                <div class="form-group">
                                    ...
                                    <select class="form-control select-search select2-hidden-accessible" data-fouc=""
                                        ...
                                        @foreach($vendors as $vendor)
                                            <option
                                                value="{{ $vendor->id }}" {{ $billPurchase->vendor_id == $vendor->id ? 'selected' : '' }}>{{ $vendor->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control select-search select2-hidden-accessible" data-fouc=""
                                        ...
                                        @foreach($methods as $method)
                                            <option value="{{ $method->id }}" {{ $billPurchase->method_id == $method->id ? 'selected' : '' }}>{{ $method->name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                            <div class="col-lg-3">
                                <div class="form-group">
                                    ...
                                    <input class="form-control" type="date" name="date"
                                           value="{{ $billPurchase->date->format('Y-m-d') }}"
                            ...
                            <div class="col-lg-10">
                                <div class="form-group">
                                    ...
                                    <input type="text" class="form-control" name="description" placeholder="الملاحظات"
                                           value="{{ $billPurchase->description }}">
                                    @error('description')
        ...
        $(document).ready(function () {
            ...
            // var crudServiceBaseUrl = "http://127.0.0.1:8000/api/",
                dataSource1 = new kendo.data.DataSource({
                    transport: {
                        read: {
                            url: crudServiceBaseUrl + "items-purchase",
                            dataType: "json",
                        },
            ...
            function addItem(e) {
                ...
                $.ajax({
                    type: 'POST',
                    url: "{{ route('item.purchase.store' , ['billId' => $billPurchase->id]) }}",
                    data: {id: dataItem.id},
            ...
            dataSource2 = new kendo.data.DataSource({
                transport: {
                    read: {
                        url: crudServiceBaseUrl + "bill/" + {{ $billPurchase->id }} + '?bill_id=' + {{ $billPurchase->id }},
                        dataType: "json",
                    },
                    update: {
                        url: crudServiceBaseUrl + "bill/update/" + {{ $billPurchase->id }},
                        dataType: "json",
                        type: "POST",
                        complete: function (e) {
                            $("#grid2").data("kendoGrid").dataSource.read();
                    ...
                    destroy: {
                        url: crudServiceBaseUrl + "bill/destroy/" + {{ $billPurchase->id }},
                        dataType: "json",
                        complete: function (e) {
                            $("#grid1").data("kendoGrid").dataSource.read();
                        }
            ...
            $("#grid2").kendoGrid({
                ...
                columns: [
                    ...
                    {field: "count", title: "العدد", width: 100},
                    {
                        field: "stock.value",
                        title: "اسم المخزن",
                        editor: stockCategoryEditor,
                        width: 100,
                    },
                    {field: "price", title: "سعر الشراء", format: "{0:n4}", width: 100},
            ...
            function stockCategoryEditor(container, options) {
